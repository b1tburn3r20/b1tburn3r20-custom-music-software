...
const MusicQueue = () => {
  ...
  return (
    <div className="w-full flex justify-end">
      <div className="flex justify-end bg-black/10 w-[25vw] h-full flex-col rounded-bl-lg">
        <div className="text-xl flex gap-1 items-center font-bold p-4 pb-0 text-muted-foreground ">
        ...
        <ScrollArea className="px-4 pt-4  h-[85vh] min-h-0  max-w-md">
          {queue.map((song, index) => <MusicQueueItem currentlyPlaying={currentlyPlaying} queue={queue} index={index} song={song} key={song.path} onPlay={handlePlay} onPause={handlePause} onResume={handleResume} isPaused={paused}
